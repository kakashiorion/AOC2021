input = ['fdgcea dgefa efabdg fdgcba dab baefc begd ebdfa db bafgdce | gfbcda cgfadb badcefg eacfgd',
...
for (element of left) {
    ...
    for (item of fives) {
        if (
            item.filter(x => dict[4].includes(x)).length == 2
        ) {
            dict[2] = item;
        }
        else if (
            item.filter(x => dict[1].includes(x)).length == 1
        ) {
            dict[5] = item;
        } else {
            dict[3] = item;
    ...
    for (item of sixes) {
        if (
            item.filter(x => dict[4].includes(x)).length == 4
        ) {
            dict[9] = item;
        }
        else if (
...
let numberList = [];
for (let k = 0; k < 200; k++) {
    rightWords = right[k].split(" ");
    let leftWordDict = leftDict[k];
    let n = "";
    for (element of rightWords) {
        for (let i in leftWordDict) {
            if (leftWordDict[i].length == element.length &&
                leftWordDict[i].every(x => element.includes(x))) {
...
let s = numberList.reduce((a, b) => a + b)
